...
interface Event { id: string; host_id: string; event_type: string; severity: string; description: string | null; timestamp: string; }
...
export default function EventsPage() {
  ...
  const [filters, setFilters] = useState({ severity: "", event_type: "", host_id: "", username: "", source_ip: "", service_name: "", status: "", q: "" });

  const load = useCallback(() => {
    const q = buildQuery({ page, page_size: pageSize, sort, ...filters }, queryParams);
    api<{ items: Event[]; total: number }>(`/api/v1/events${q}`).then((r) => { setEvents(r.items); setTotal(r.total); }).catch(console.error);
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-4 gap-2 mb-4">
        ...
        <input placeholder="Event type" value={filters.event_type} onChange={(e) => setFilters({ ...filters, event_type: e.target.value })} className={inputCls} />
        <input placeholder="Username" value={filters.username} onChange={(e) => setFilters({ ...filters, username: e.target.value })} className={inputCls} />
        <input placeholder="Source IP" value={filters.source_ip} onChange={(e) => setFilters({ ...filters, source_ip: e.target.value })} className={inputCls} />
        <input placeholder="Service" value={filters.service_name} onChange={(e) => setFilters({ ...filters, service_name: e.target.value })} className={inputCls} />
        <input placeholder="Status" value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })} className={inputCls} />
      ...
      <table className="w-full text-sm">
        <thead><tr className="text-left text-gray-500 border-b border-[var(--border)]">
          <th className="py-2">Time</th><th className="py-2">Type</th><th className="py-2">Severity</th><th className="py-2">Description</th>
        </tr></thead>
        <tbody>
          {events.map((e) => (
