...
test.describe("Threat intel reference set CRUD", () => {
  ...
  test("create reference set, add entry, disable, delete", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Threat Intel" })).toBeVisible({ timeout: 30_000 });
    await expect(page.getByRole("button", { name: "Create set" })).toBeVisible({ timeout: 15_000 });
    ...
    const card = page.locator("div.p-3.rounded.border").filter({ has: page.getByText(setName, { exact: true }) }).first();
    ...
    await expect(page.getByRole("button", { name: new RegExp(`^${setName}\\b`) })).not.toBeVisible({ timeout: 15_000 });
  });
});
