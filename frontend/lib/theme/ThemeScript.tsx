...
export function ThemeScript({ nonce }: { nonce?: string }) {
  const script = `
    (function() {
      try {
        var t = localStorage.getItem('securi-theme') || localStorage.getItem('securisphere-theme');
        var theme = t === 'light' ? 'light' : 'dark';
        document.documentElement.setAttribute('data-theme', theme);
        document.documentElement.classList.add(theme);
        var m = localStorage.getItem('securi-reduced-motion') || localStorage.getItem('securisphere-reduced-motion');
