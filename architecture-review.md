# Securi — Complete Architecture Review

> **Reviewed:** 2026-08-05
> **Scope:** Full repository (backend, frontend, agent, infrastructure)
> **Codebase size:** ~600+ source/config files, ~210 Python modules, ~96 React components, ~180 API endpoints
...
### C4. Duplicate Auth Page Routes — Dead Code
**Files:** `frontend/app/login/`, `frontend/app/register/`, `frontend/app/forgot-password/`, `frontend/app/reset-password/` AND `frontend/app/(auth)/login/`, `frontend/app/(auth)/register/`, etc.
**Category:** Dead Code / Broken Structure

Both root-level and `(auth)` route group versions exist. In Next.js App Router, route groups `(auth)` are transparent — so `/login` matches BOTH `app/login/page.tsx` and `app/(auth)/login/page.tsx`. This creates ambiguity and potential routing conflicts.

**Action:** Remove the root-level duplicates and keep only the `(auth)/` group versions.
